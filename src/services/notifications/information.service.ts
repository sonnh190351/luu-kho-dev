...
export class InformationService {
   ...
   async showCategoryDetails(id: number){
      const category = await DatabaseService.getInstance().getByField(DatabaseTables.Categories, 'id', String(id))
      
      if(category.error){
         NotificationsService.error("Failed to get category details", category.error.toString())
         return
      }
